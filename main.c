    ...
    for (i = 0; i < 8; i++) {
      switch (evaluation(state, state->x, state->y, i)) {
        case BATTLE:
          ...
          switch (state->battle.resul =
                      battle(state, state->battle.att, state->battle.def)) {
            case WIN:
              redraw(state);
...
int selection(struct state_t *state) {
  do
    state->selection = (state->selection + 1) % 5 + 5 * (state->turn % 2);
...
int insertcard(struct state_t *state, int eq, int num) {
  if (state->table[state->x][state->y] == 0 &&
      state->cards[eq][num].played == 0) {
    state->table[state->x][state->y] =
        num + 2 + 5 * eq;  // dos estados para vacios
...
enum evaluation_e evaluation(struct state_t *state, int x, int y, int i) {
  ...
  if (((state->cards[eq][num].arrows & 1 << i) >> i) &
      itojkxy(i, &j, &k, x, y)) {
    // Comprueba cada flecha que tiene la carta y si la casilla adyacente es una
    // casilla jugable
    if (state->table[x + j][y + k] > 1) {
      // La pongo aquí no vaya a ser que vaya a intentar evaluarla para valores
      // mas grandes de 4 o mas chicos que 0. Comprueba que en la posición
      // adyacente hay una carta.
      ...
      if (state->cards[eq][num].eq != state->cards[eq_en][num_en].eq) {
        ...
        if (eq == eq_old) {
          // Comprueba si el atacante sigue siendo del mismo equipo que al
          // principio
          state->battle.att = &state->cards[eq][num];
          state->battle.def = &state->cards[eq_en][num_en];
          for (l = 0; l < 8; l++) {
            // comprueba si hay flechas que apunten de la defensora a la carta
            // de ataque
            state->evaluation.d_arr = l;
            if (((state->cards[eq_en][num_en].arrows & 1 << l) >> l) &&
                itojk(l, &m, &n)) {
          ...
          if (a == 0) {
            // En caso de no tener flechas con las que defenderse, la carta se
...
enum battle_e battle(struct state_t *state, struct card_t *att,
                     struct card_t *def) {
  ...
  switch (att->type) {
    ...
    case X:
      ...
      break;
    case A:
      state->battle.param_def =
          min(min(def->stats[0], def->stats[1]), def->stats[2]);
      if (att->stats[0] >
          min(min(def->stats[0], def->stats[1]), def->stats[2])) {
        return WIN;
      }
...
int combi(struct state_t *state, struct card_t *card) {
  ...
  for (i = 0; i < 8; i++) {
    ...
    // Comprueba cada flecha que tiene la carta y si la casilla adyacente es una
    // casilla jugable
    if (((card->arrows & 1 << i) >> i) &&
        itojkxy(i, &j, &k, card->x, card->y)) {
      // La pongo aquí no vaya a ser que vaya a intentar evaluarla para valores
      // mas grandes de 4 o mas chicos que 0. Comprueba que en la posición
      // adyacente hay una carta.
