...
int redraw(struct state_t *state) {
  if (state->phase == SELECTION_P) {
    ...
    draw_cursor_b(winx + 44 - (44 + 12) * (state->selection / 5) - 1,
                  winy + 6 * (state->selection % 5) - 1, 6);
    int i, j;
    for (i = 0; i < 4; i++) {
      for (j = 0; j < 4; j++) {
        if (state->table[i][j] > 1) {
          draw_card(&state->cards[0][state->table[i][j] - 2], winx + 2 + 10 * i,
                    winy + 2 + 6 * j);
        }
        if (state->table[i][j] > 6) {
          draw_card(&state->cards[1][state->table[i][j] - 7], winx + 2 + 10 * i,
                    winy + 2 + 6 * j);
        }
      }
    }
    for (i = 0; i < 5; i++) {
      if (state->cards[0][i].played == 0)
        draw_card(&state->cards[0][i], winx + 44, winy + 6 * i);
      if (state->cards[1][i].played == 0)
        draw_card(&state->cards[1][i], winx - 12, winy + 6 * i);
    }
  }
  if (state->phase == BATTLE_P) {
    draw_card(state->battle.att, winx + 2 + 10 * state->battle.att->x,
              winy + 2 + 6 * state->battle.att->y);
    draw_arrows(state->evaluation.a_arr, winx + 2 + 10 * state->battle.att->x,
                winy + 2 + 6 * state->battle.att->y, 5);
    move(20, 10);
    printw("Batalla ");
